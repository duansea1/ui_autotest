...
class Base:
    ...
    def find_childs_use_ele(self, father_ele, children_loc):
        '''
        @param father_ele: 父元素
        @return: 子元素列表 /空列表'''
        try:
            child_ele=father_ele.find_elements(*children_loc[:2])
            return child_ele
        except NoSuchElementException:
            msg='查找子元素超时：'+str(children_loc)
            print(msg)
    ...
    def find_childs_use_loc(self, father_loc, children_loc):
        '''
        @param father_loc: 父元素定位方式
        @return: 子元素列表 /空列表
        '''

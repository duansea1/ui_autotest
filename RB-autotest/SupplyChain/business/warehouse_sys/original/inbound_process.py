...
from wms_inbound_process import inbound_resource as R
...
class Warehouse_inbound(Base):
    ...
    def inbound_process(self, ASN_num, warehouse_id):
        ...
        # 登陆
        home_URL = Warehouse_inbound.inbound_login(self, warehouse_id)
        assert 'home.xhtml' in home_URL

        # ASN状态查询
        ASN_type = Warehouse_inbound.inbound_ASNstatus(self, ASN_num)[0]
        ASN_status = Warehouse_inbound.inbound_ASNstatus(self, ASN_num)[1]
        if 'DO退货' in ASN_type:
            if '初始化' in ASN_status:
                # 入库管理-验收
                accept_message = Warehouse_inbound.inbound_accpet_DOreturned(self, ASN_num)
                print(accept_message)
                if '操作成功' in accept_message:
                    # 入库管理-上架
                    on_shelves_status = Warehouse_inbound.inbound_on_shelves(self, ASN_num)
                    print(on_shelves_status)
                    if '已完成' in on_shelves_status:
                        # 入库管理-ASN审核
                        ASN_status = Warehouse_inbound.inbound_aduit(self, ASN_num)
                        print(ASN_status)
                        if '完成' in ASN_status:
                            print('入库完成，关闭浏览器')
                            self.driver.quit()
    ...
    def inbound_accpet_DOreturned(self, ASN_num):
        """入库管理-验收(DO退货)"""
        print('入库管理-验收')
        ...
        for i in range(0, len(trs), 1):
            if ASN_row_status[i] == '初始化':
                ...
                self.driver.find_element_by_id('searchForm:productBarcode').send_keys()
                self.driver.find_element_by_id('searchForm:productBarcode').send_keys(Keys.ENTER)
                # self.send_keys(R.Accept.product_barcode, product_id[i])
                # self.send_keys(R.Accept.product_barcode, Keys.ENTER)
                sleep(3)
                # receive_qty = self.find_element_by_css_selector("div#searchForm:j_id309>label[2]").text
                # # expect_qty = self.find_elements_by_id('searchForm:j_id309')[i]
                # # receive_qty = expect_qty.find_elements_by_tag_name('label')[2].text
                # print('应收数量为' + str(receive_qty))
                ...
                # self.driver.switch_to_window(main_handle)
                sleep(1)
                self.find_element_by_class_name('batchNo').send_keys(Keys.ENTER)
                sleep(1)
    ...
    # def inbound_accept_purchasing(driver, ASN_num):
    # """入库管理-验收(正采)"""
    #
    #     return
    ...
    def inbound_aduit(self, ASN_num):
        """入库管理-ASN审核"""
        print('进入入库管理-ASN查询')
        self.driver.get('http://10.6.80.248:8080/receive/asnList.xhtml')
        sleep(1)

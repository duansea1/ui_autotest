...
from wms_outbound_process import outbound_resource as R
...
class Warehouse_outbound(Base):
    ...
    def outbound_process(self, warehouse_id, do_num, isYCorder):
        ...
        # 登陆
        Warehouse_outbound.outbound_login(self, warehouse_id)
        # 查询do_num状态
        do_status = Warehouse_outbound.outbound_delivery_order(self, do_num, isYCorder)
        if '初始化' in do_status:
            Warehouse_outbound.outbound_initial_process(self, do_num, isYCorder)
        elif '分配完成' in do_status:
            Warehouse_outbound.outbound_dispatched_process(self, do_num, isYCorder)
        elif '拣货完成' in do_status:
            Warehouse_outbound.outbound_confirmed_process(self, do_num, isYCorder)
        elif '分拣完成' in do_status:
            Warehouse_outbound.outbound_sorted_process(self, do_num)
        elif '装箱完成' in do_status:
            Warehouse_outbound.outbound_encaseed_process(self, do_num)
        else:
            print('DO状态异常')
        return
    ...
    def outbound_initial_process(self, do_num, isYCorder):
        """状态为初始化的do_num"""
    # 订单分配
        order_dispatch_status = Warehouse_outbound.outbound_order_distribution(self, do_num)
        if "分配成功" in order_dispatch_status:
            # 生成波次
            wave_status = Warehouse_outbound.outbound_wave_generation(self, do_num, isYCorder)
            if "波次生成成功" in wave_status:
                # 波次计划
                wave_id = wave_status.split(":")[1]
                pick_id = Warehouse_outbound.outbound_wave_plan(self, wave_id, isYCorder)
                if pick_id != None:
                    # 拣货确认
                    confirm_status = Warehouse_outbound.outbound_pick_confirm(self, pick_id)
                    if '操作成功' in confirm_status:
                        # 分拣
                        sorting_status = Warehouse_outbound.outbound_sorting(self, wave_id)
                        if '分拣成功' in sorting_status:
                            # 核拣装箱
                            carton_no = Warehouse_outbound.outbound_encasement(self, do_num)
                            if carton_no != None:
                                # 出库交接
                                handover_status = Warehouse_outbound.outbound_handover(self, carton_no)
    ...
    def outbound_dispatched_process(self, do_num, isYCorder):
        """状态为分配完成的do_num"""
        wave_id = Warehouse_outbound.outbound_getwave_id(self, do_num, isYCorder)
        if wave_id == None:
            # 生成波次
            wave_status = Warehouse_outbound.outbound_wave_generation(self, do_num, isYCorder)
            if "波次生成成功" in wave_status:
    ...
    def outbound_encaseed_process(self, do_num):
        """状态为装箱完成的do_num"""
        # 获取箱号
        carton_no = Warehouse_outbound.outbound_getcarbon_no(self, do_num)
        if carton_no != None:
            # 出库交接
            handover_status = Warehouse_outbound.outbound_handover(self, carton_no)
            if '已发货' in handover_status:
    ...
    def outbound_getcarbon_no(self, do_num):
        ...
        self.find_element(R.Encasement.order_no, do_num)
    ...
    def outbound_delivery_order(self, do_num, isYCorder):
        ...
        if len(result) == 0:
            ...
            return self.find_element(R.DeliveryOrder.do_status).text
    ...
    def outbound_encasement(self, do_num):
        ...
        print('产品条码是' + product_id)
        print('点击关闭窗口按钮')
        self.find_element(R.Encasement.close_button).click()
        # self.driver.find_element_by_id('hidelink').click()
        sleep(1)
        ...
        self.send_keys(R.Encasement.product_input, product_id)
...
if __name__ == '__main__':
    ...
    testOut.outbound_process('13', '7029629909', 'true')

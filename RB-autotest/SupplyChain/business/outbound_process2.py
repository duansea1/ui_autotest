...
Modified by chenpeng @ 2019.5.20
...
class Warehouse_outbound(Base):
    ...
    def outbound_process(self, warehouse_id, do_num, isYCorder,outbound_type):
        ...
        '''改为通过数据库查询DO状态'''
        query=self.query_do_status(do_num)
        do_status =query [0]
        warehouse_no=query[1]
        assert warehouse_id == warehouse_no, print('仓库选择错误')
        sleep(2)
        # 登陆
        ...
        elif '交接完成' or '交接中' in do_status:
            print('当前订单正在自动出库，请耐心等待2-3分钟')
        else:
            assert print('DO状态异常')
        return
    ...
    # 订单分配
        ...
        if "波次生成成功" in wave_status:
            ...
            if pick_id != None:
                ...
                # do_status = self.outbound_delivery_order(do_num, isYCorder)
                '''改为通过数据库查询DO状态'''
                do_status = self.query_do_status(do_num)[0]
                ...
                if u'拣货完成' in do_status:
                    ...
                    if '分拣成功' in sorting_status:
                            # 核拣装箱
                        self.outbound_encasement(do_num,outbound_type)
                        # if carton_no != None:
                            # if carton_no == None:
                                # 出库交接
                        # carton_no = self.outbound_getcarbon_no(do_num)
                        '''改为通过数据库查询箱号'''
                        carton_no = self.query_carton_num (do_num)
                        handover_status = self.outbound_handover(do_num,carton_no,outbound_type)
                        if '已发货' or '自动交接' in handover_status:
                            print('出库完成，关闭浏览器')
                            self.driver.quit()
                        else:
                            assert print('交接失败！')
    ...
    def outbound_dispatched_process(self, do_num, isYCorder, outbound_type):
        """状态为分配完成的do_num"""
        #self.outbound_wave_generation(do_num,isYCorder,outbound_type)
        #wave_id = self.outbound_getwave_id(do_num, isYCorder)
        '''改为通过数据库查询波次号'''
        wave_id=self.query_do_status(do_num)[2]
        print('波次编码为： '+ wave_id)
    ...
    def outbound_delivery_order(self, do_num, isYCorder):
        ...
        self.find_element(R.DeliveryOrder.search_button).click()
        '''
        self.click(R.DeliveryOrder.search_button)
        print('点击查询,显示结果')
        #   self.find_element_by_css_selector('#searchForm>table>tbody>tr:nth-child(3)>td:nth-child(11)>input').click()
    ...
    def outbound_wave_generation(self, do_num, isYCorder, outbound_type):
        ...
        #no_data=self.find_element(R.WaveGeneration.no_data_pic)
        # result = self.find_elements_by_xpath("//tbody[@id='selectForm:resultTable:tb']/tr")
        #if len(result) == 0:
        wms_handle=self.driver.current_window_handle
        if outbound_type == '1':
            try:
                if self.find_element(R.WaveGeneration.no_data_pic).is_displayed():#没有数据的图片
                    print('未查询到结果,重新指派承运商')
                    warehouse_id=self.query_do_status(do_num)[1]
                    self.assign_new_carrier_by_warehouse(do_num,warehouse_id)
                    self.driver.close()
                    #self.driver.find_element_by_name("searchForm:j_id259").send_keys(Keys.CONTROL,'t')
                    sleep(2)
                    # all_handles=self.driver.window_handles
                    # print(all_handles)
                    # for handle in all_handles:
                    #     if handle != wms_handle:
                    #         #handle =wms_handle
                    #         print('切换到TMS窗口:'+handle)
                    #         self.driver.switch_to_window(handle)
                    # #self.driver.get("http://tms2.111.com.cn/home")
                    print('切换到WMS窗口')
                    self.driver.switch_to_window(wms_handle)
                    #tms_handle=self.driver.current_window_handle
                    #all_handles=self.driver.window_handles
                    # for handle in all_handles:
                    #     if handle != tms_handle:
                    #         #handle =wms_handle
                    #         print('切换到WMS窗口')
    ...
    def outbound_encasement(self, do_num,outbound_type):
        """出库管理-核拣装箱"""
        print('打开核拣装箱页面')
        self.driver.get('http://10.6.80.248:8080/delivery/reCheck.xhtml')
        sleep(1)
        ...
        self.driver.switch_to_alert().accept()
        sleep(1)
        product_nocheck_count=self.find_element(R.Encasement.product_nocheck_count).text
        print('未核拣数为:' + product_nocheck_count)
        if product_nocheck_count == '0' :
            print('更新DO状态为装箱完成')
            #订单状态；00 - 初始化，30 - 分配中，40 - 分配完成，50 - 拣货中，60 - 拣货完成，64 - 分拣中，65 - 分拣完成，66 - 装箱中，67-装箱完成，68 - 交接中，69 - 交接完成，80 - 已出库，90 - 已取消
            self.update_do_status(do_num,"67",outbound_type)

            if outbound_type == '1':
                print ("装箱完成，等待1-2分钟自动交接即可出库")

            else:
                print('继续RTV交接 or TO交接')


    def outbound_handover(self,do_num,carton_no,outbound_type):
        """出库管理-出库交接"""
        print('打开出库交接页面')
        ...
        if outbound_type == '2':
            ...
            supplier_name=self.query_supplier_name(do_num)
            print('输入供应商:'+supplier_name)
            #self.send_keys(R.Handover.supply,'陈氏药业')
            self.send_keys(R.Handover.supply,supplier_name)
            print('输入回车')
        ...
        if outbound_type =='2' or outbound_type =='3':
            print('点击保存')
            ...
            handover_status = '自动交接'
        return handover_status

    def update_do_status(self,do_num,do_status,outbound_type):
        '''PC核减装箱不更新DO状态，连接数据库更新'''
        ...
        select_status='select do_no,status from doc_do_header where do_no=to_char(%s) '% (do_num)
        select_waybill='select way_bill from doc_carton_header a inner  join doc_do_header b on a.do_header_id=b.id where b.do_no=to_char(%s)'% (do_num)
        update_dostatus = 'update doc_do_header set status=to_char(%s) where do_no=to_char(%s) '% (do_status, do_num)
        update_tracking_num = "update doc_carton_header  set  way_bill=(select tracking_no from doc_do_wave_ex where do_h_id=(select id from doc_do_header where do_no=to_char(%s))) " \
                              "where do_header_id=(select id from doc_do_header where do_no=to_char(%s))" % (
                              do_num, do_num)
        if outbound_type == '1':
            cursor.execute(update_tracking_num)
            wms_oracle.commit ()
            cursor.execute (select_waybill)
            result1 = cursor.fetchall ()
            assert len (result1) != 0, print ('当前DO运单号为空，请在TMS指派新的承运商，建议为药城德邦or京东物流')
            waybill_no = (result1[0])[0]
            print('快递单号为：'+waybill_no)
        else:
            pass
        cursor.execute(update_dostatus)
        wms_oracle.commit()
        cursor.execute(select_status)
        result2 = cursor.fetchall()
        do = (result2[0])[0]
        status = (result2[0])[1]
        assert do_status == status
        print(do+'当前状态已更新为：'+status)
        wms_oracle.close()

    def query_carton_num(self,do_num):
        '''通过数据库查询箱号'''
        wms_oracle = oracle.connect ('WMS_SH_YW', 'WMS_SH_YW', '10.6.84.20:1521/WMSOracle')
        # 用自己的实际数据库用户名、密码、主机ip地址 替换即可
        print ('WMS-Oracle数据库:连接成功')
        cursor = wms_oracle.cursor ()  # 使用cursor()方法获取操作游标
        select_carton_no = "select a.carton_no from doc_carton_header a " \
                 "inner join doc_do_header b on a.do_header_id=b.id " \
                 "where b.do_no=to_char(%s)" % (do_num)
        cursor.execute (select_carton_no)
        result1 = cursor.fetchall ()
        carton_no = (result1[0])[0]
        print ('箱号为：' + carton_no)
        wms_oracle.close()
        return carton_no

    def query_do_status(self,do_num):
        '''通过数据库查询DO状态+仓库ID+波次号'''
        # 订单状态；00 - 初始化，30 - 分配中，40 - 分配完成，50 - 拣货中，60 - 拣货完成，64 - 分拣中，65 - 分拣完成，66 - 装箱中，67-装箱完成，68 - 交接中，69 - 交接完成，80 - 已出库，90 - 已取消
        wms_oracle = oracle.connect ('WMS_SH_YW', 'WMS_SH_YW', '10.6.84.20:1521/WMSOracle')
        # 用自己的实际数据库用户名、密码、主机ip地址 替换即可
        print ('WMS-Oracle数据库:连接成功')
        cursor = wms_oracle.cursor ()  # 使用cursor()方法获取操作游标
        select_do_status="select status,warehouse_id from doc_do_header where do_no=to_char(%s)"%(do_num)
        select_waveno = 'select wave_no from doc_wave_header where id = (select wave_id from doc_do_header where do_no=to_char(%s))'%(do_num)
        cursor.execute(select_do_status)
        result=cursor.fetchall()
        do_status_no=(result[0])[0]
        warehouse_id=str((result[0])[1])
        cursor.execute (select_waveno)
        ...
        if do_status_no =='00':
            do_status = '初始化'
        elif do_status_no == '30' :
            do_status = '分配中'
        elif do_status_no == '40' :
            do_status = '分配完成'
        elif do_status_no == '50' :
            do_status = '拣货中'
        elif do_status_no == '60' :
            do_status = '拣货完成'
        elif do_status_no == '64' :
            do_status = '分拣中'
        elif do_status_no == '65' :
            do_status = '分拣完成'
        elif do_status_no == '67':
            do_status = '装箱完成'
        elif do_status_no == '68':
            do_status = '交接中'
        elif do_status_no == '69':
            do_status = '交接完成'
        elif do_status_no == '80':
            do_status = '已出库'
        elif do_status_no == '90':
            do_status = '已取消'
        else:
            assert print ('DO状态异常')
        print(do_num+'-状态为：'+ do_status)
        wms_oracle.close()
        print('WMS-Oracle数据库:断开连接')
        return do_status,warehouse_id,wave_no

    def assign_new_carrier_by_warehouse(self,do_num,warehouse_id):
        """指派新的承运商"""
        js = 'window.open("http://tms2.111.com.cn/home");'
        self.driver.execute_script (js)
        all_handle = self.driver.window_handles
        # 获取当前页句柄
        tms_handle=all_handle[1]
        print ('切换到TMS窗口:' + tms_handle)
        self.driver.switch_to_window (tms_handle)
        print ('进入TMS 配送管理系统')
        account = read_json ('account', source='manage')
        id = account['name']
        pin = account['password']
        self.send_keys (R.WaveGeneration.user, id)
        self.send_keys (R.WaveGeneration.pwd, pin)
        self.click (R.WaveGeneration.login_button)
        sleep (1)
        ...
        sleep(2)
        print ("切换主界面frame")
        frame=self.find_element(R.WaveGeneration.frame)
        self.driver.switch_to.frame (frame)
        print ("输入DO单号：" + do_num)
        self.send_keys (R.WaveGeneration.do_input,do_num)
        print ("清空下单时间")
        sleep (2)
        self.driver.find_element_by_id ("handleTimeStart").clear ()
        sleep (2)
        self.driver.find_element_by_id ("handleTimeEnd").clear ()
        sleep (1)
        self.find_element(R.WaveGeneration.do_input).click ()
        print ("点击查询")
        sleep (1)
        self.find_element(R.WaveGeneration.query_btn).click ()
        sleep (1)
        carrier1 = self.driver.find_element_by_xpath ('//table/tbody/tr[1]/td[7]/div').text
        print ("当前承运商为：" + carrier1)
        print ("勾选承运商")
        check_box=self.find_element(R.WaveGeneration.check_box)
        check_box.click()
        sleep (1)
        print ("点击指派")
        self.find_element(R.WaveGeneration.assign_btn).click ()
        print ("切换到弹窗frame")
        popup_frame=self.find_element(R.WaveGeneration.popup_frame)
        self.driver.switch_to.frame (popup_frame)
        sleep (1)
        self.find_element (R.WaveGeneration.carrier_choose).click ()
        sleep (1)
        if warehouse_id == '13':#广州仓
            print('选择承运商-广州药城德邦')
            self.find_element(R.WaveGeneration.gz_debang).click()
        elif warehouse_id == '30' :#昆山药城
            print ('选择承运商-昆山药城德邦')
            self.find_element (R.WaveGeneration.ks_debang).click ()
        elif warehouse_id == '20' :#重庆仓
            print ('选择承运商-重庆药城德邦')
            self.find_element (R.WaveGeneration.cq_debang).click ()
        elif warehouse_id == '15':#昆山药网
            print ('选择承运商-京东物流药网')
            self.find_element (R.WaveGeneration.cq_debang).click ()
        elif warehouse_id == '10':#天津仓
            print('选择承运商-圆通药网')
            self.find_element(R.WaveGeneration.yw_yuantong).click()
        else:
            print('选择承运商-圆通药网')
            self.find_element(R.WaveGeneration.yw_yuantong).click()
            sleep(1)
        print('指派')
        self.click(R.WaveGeneration.assign)
        sleep (2)
        ...
        self.driver.switch_to.frame (frame)
        sleep (2)
        carrier2 = self.driver.find_element_by_xpath ('//table/tbody/tr[1]/td[7]/div').text
        print ("新承运商为：" + carrier2)
        sleep(1)
    def query_supplier_name(self,do_num):
        '''通过数据库查询箱号'''
        wms_oracle = oracle.connect ('WMS_SH_YW', 'WMS_SH_YW', '10.6.84.20:1521/WMSOracle')
        # 用自己的实际数据库用户名、密码、主机ip地址 替换即可
        cursor = wms_oracle.cursor ()  # 使用cursor()方法获取操作游标
        select_supplier_name = 'select consignee_name  from doc_do_header where do_no=to_char(%s)' % (do_num)
        cursor.execute (select_supplier_name)
        result2 = cursor.fetchall ()
        supplier_name = (result2[0])[0]
        print ('供应商为：' + supplier_name)
        wms_oracle.close()

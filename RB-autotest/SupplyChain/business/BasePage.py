...
class Base:
    ...
    def find_element(self, ele_loc):
        ...
        '''
        try:
            self.wait_until_display(ele_loc)
    ...
    def find_elements(self, ele_loc):
        '''查找元素
        @param loc: 元素定位方法，类似： (By.ID, "bu")
        @param ele_desc: 元素描述'''
        try:
            WebDriverWait(self.driver,30).until(EC.visibility_of_any_elements_located(ele_loc[:2]),message='timeout')

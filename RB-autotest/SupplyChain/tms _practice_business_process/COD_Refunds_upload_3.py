...
print('选择已返款订单')
browser.find_element_by_xpath('//*[@id="ra"]/div[2]/div/div/dl/dd[4]').click()
sleep(1)
print("输入出库时间")
...
if refunds1==refunds2 and refunds2==i:
    if status2 =="已返款":
        print('已返款订单不能重复返款，测试失败')
elif refunds1 == refunds2 and refunds2 !=i :
    if status2 == '已返款':
        print('当前订单已返款成功，测试通过')
else:
    if status2 !='已返款':
        print('测试不通过')
...
from business.pms.MerchantComsManagement import MerchantComsManagement
...
#     h.go_merchant_coms_management()
#     yield dr
#     dr.quit()

@pytest.mark.pms_0
def test_search_by_apply_code(selenium):
    """菜单：商家管理--商家佣金管理，按申请编码查询功能正确"""
...
#     mgr.chose_subsystem('采购管理系统(集团)','系统仓库')     #进入-采购管理系统(集团)
    ...
    h=Home(selenium)     #PMS
    h.go_merchant_coms_management()
    s =MerchantComsManagement(selenium)
    s.get_search_result()
    ...
    table_item = s.find_element(R.merchant_coms_management.table_items)

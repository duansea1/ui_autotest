...
# @Author  : wanghui
import pytest
from business.pms.ContractSync import ContractSync
...
#     mgr.chose_subsystem('采购管理系统(集团)','系统仓库')     #进入-采购管理系统(集团)
    ...
    h=Home(selenium)     #PMS
    h.go_contract_sync()
    p = ContractSync(selenium)
    contractCode=p.driver.find_element_by_xpath('//*[@id="rebateContractSyncList"]/tbody/tr[1]/td[3]').text
    p.search_by_contractCode(contractCode)
    table_item = p.find_element(R.contract_sync.table_items)
    table_items_rows = table_item.find_elements_by_tag_name('tr')
    #print(len(table_items_rows))
    assert len(table_items_rows)>1

...
# @Author  : wanghui
import pytest
from business.pms.Supplier import Supplier
...
# @pytest.mark.pms_0
# def test_supplier(chrome_dr):
#     """菜单：供应商管理，查询功能正确"""
#     s = Supplier(chrome_dr)
#     s.get_search_result()
...
#     mgr.chose_subsystem('采购管理系统(集团)','系统仓库')     #进入-采购管理系统(集团)
    ...
    s = Supplier(selenium)
    s.get_search_result()
    supplier_name = s.driver.find_element_by_xpath('//*[@id="maingrid4|2|r1001|c107"]/div').text
    s.get_result_by_supplierName(supplier_name)
    page_source = s.get_page_source()
    assert '保证金录入' in page_source
...
def test_search_by_supplierCode(selenium):
    ...
    s = Supplier(selenium)
    s.get_search_result()
    supplier_code = s.driver.find_element_by_xpath('//*[@id="maingrid4|2|r1001|c106"]/div').text
    s.get_result_by_supplierCode(supplier_code)
    page_source = s.get_page_source()
    assert '保证金录入' in page_source
...
def test_search_by_supplierStatus(selenium):
    ...
    s = Supplier(selenium)
    s.get_search_result()
    s.get_result_by_supplierStatus('3')
    page_source = s.get_page_source()
    assert '保证金录入' in page_source

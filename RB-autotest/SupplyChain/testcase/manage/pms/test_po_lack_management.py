...
from business.pms.PoLackManagement import PoLackManagement
...
#     mgr.chose_subsystem('采购管理系统(集团)','系统仓库')     #进入-采购管理系统(集团)
    ...
    h=Home(selenium)     #PMS
    h.go_po_lack_management()
    p = PoLackManagement(selenium)
    p.get_search_all()
    table_item = p.find_element(R.po_lack_management.table_items)

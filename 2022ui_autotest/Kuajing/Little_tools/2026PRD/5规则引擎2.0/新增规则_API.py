...
def create_rule(rule_number):
    ...
    参数说明:
        rule_number: 规则编号尾号 (如: 4 表示 USD_SEA_4)
    ...
    script_config = {
        "conditionGroups": [{
            ...
            "conditions": [{
                ...
                "fieldType": "java.lang.Number",
                "condition": "=",
                "symbol": "AND",
                "expectedValue": str(rule_number),
                "expression": "",
                "function": "",
                "functionParams": "",
                "useFunction": 0,
    ...
    payload = {
        ...
        "scriptConfig": json.dumps(script_config, ensure_ascii=False),
        "scriptContent": "",
        "inputParamsType": json.dumps(input_params_type, ensure_ascii=False),
        "ruleName": f"{param_code} = {rule_number}",
        "ruleDesc": f"{param_code} = {rule_number}",
        "version": "",
        "riskFlagHit": "yes",
    ...
    headers = {
        ...
        "sec-ch-ua-mobile": "?0",
        "User-Agent": "Mozilla/5.0 (Macintosh; Intel Mac OS X 14; rv:135.0) Gecko/20000101 Firefox/135.0",
        "Origin": f"{BASE_URL}",
...
def parse_rule_response(r, rule_number):
    """解析新增规则API响应"""
    if r.status_code == 200:
        try:
            ...
            if code == "000000":
                print(f"{C_GREEN}✅ USD_SEA_{rule_number} 创建成功{C_RESET}")
            else:
                print(f"{C_RED}❌ USD_SEA_{rule_number} 创建失败: {msg}{C_RESET}")
        except:
            print(f"{C_RED}❌ USD_SEA_{rule_number} 响应解析失败{C_RESET}")
    else:
        print(f"{C_RED}❌ USD_SEA_{rule_number} 请求失败，状态码: {r.status_code}{C_RESET}")


# ==================== 批量新增规则 ====================
def batch_create_rules(start_num, end_num, prefix="USD_SEA"):
    """
    批量新增规则
    ...
    print(f"\n🚀 开始批量新增规则 {prefix}_{start_num} ~ {prefix}_{end_num}")
    print("=" * 60)

    for i in range(start_num, end_num + 1):
        print(f"\n[{i}/{end_num}] 正在创建 {prefix}_{i}...")
        ...
        if r.status_code == 200:
            try:
                ...
                if code == "000000" or data.get("ruleId"):
                    print(f"{C_GREEN}✅ {prefix}_{i} 创建成功{C_RESET}")
                    success_count += 1
                else:
                    print(f"{C_RED}❌ {prefix}_{i} 创建失败: code={code}, msg={msg}{C_RESET}")
                    # 打印完整响应便于调试
                    print(f"   完整响应: {json.dumps(resp, ensure_ascii=False)}")
                    fail_count += 1
            except Exception as e:
                print(f"{C_RED}❌ {prefix}_{i} 响应解析失败: {e}{C_RESET}")
                fail_count += 1
        else:
            print(f"{C_RED}❌ {prefix}_{i} 请求失败，状态码: {r.status_code}{C_RESET}")
            fail_count += 1
...
if __name__ == "__main__":
    ...
    print(f"\n📊 配置信息")
    print(f"  将新增规则: {prefix}_{start_num} ~ {prefix}_{end_num}")
    print("-" * 60)

    # batch_create_rules(start_num=start_num, end_num=end_num, prefix=prefix)

...
base_url = " 	http://10.254.77.173:21103/accountFxFloatConfig/queryRate"
...
def execute_rate_calculations(req_dto, req_dto1):
    """ [验证缓存情况]
    连续两次调用 query_rate_cal_merchant_amount 方法，
    分别使用 req_dto 和 req_dto1 作为参数。

    :param req_dto: 第一次调用的请求对象
    :param req_dto1: 第二次调用的请求对象
    """
    # 设置Decimal精度
    getcontext().prec = 8

    # 第一次调用
    db_result = query_config_from_db(req_dto, env='UAT')
    if db_result:
        ...
        print("⚠️没有找到符合条件的配置⚠")
    result_first_call = query_rate_cal_merchant_amount(req_dto)
    if result_first_call.get("success", None):
        print("🎉第一次调用API成功，查询到的信息:", result_first_call)

        direction = find_real_ccy_pair(req_dto.accountCcy, req_dto.merchantCcy, env='UAT')
        calculate_merchant_amount(direction, channel_amount=req_dto.accountAmt,
                                  exchange_rate=result_first_call['result'].get('rate', 0),
                                  fluctuation=float(db_result.get("FLOAT_PERCENT") or 0),
                                  channel_ccy=req_dto.accountCcy, merchant_ccy=req_dto.merchantCcy)
    else:
        print("⚠️第一次汇率查询计算失败：", result_first_call)

    # 第二次调用
    db_result = query_config_from_db(req_dto1, env='UAT')
    if db_result:
        print("匹配的配置信息", db_result)
        print("🎉匹配的汇率渠道🎉", db_result.get("RATE_RESOURCE", ""))
        print("🎉匹配的浮动值🎉", db_result.get("FLOAT_PERCENT", ""))
        ...
        print("⚠️没有找到符合条件的配置⚠")
    result_second_call = query_rate_cal_merchant_amount(req_dto1)
    if result_second_call.get("success", None):
        print("🎉第二次调用API成功，查询到的信息:", result_second_call)

        direction = find_real_ccy_pair(req_dto1.accountCcy, req_dto1.merchantCcy, env='UAT')
        calculate_merchant_amount(direction, channel_amount=req_dto1.accountAmt,
                                  exchange_rate=result_second_call['result'].get('rate', 0),
                                  fluctuation=float(db_result.get("FLOAT_PERCENT") or 0),
                                  channel_ccy=req_dto1.accountCcy, merchant_ccy=req_dto1.merchantCcy)
    else:
        print("⚠️第二次汇率查询计算失败：", result_second_call)
...
def query_rate_info(rateId: str) -> dict:
    ...
    """
    base_url = "http://10.254.77.173:21103/accountFxFloatConfig/queryAcctRate"
    req_dto = {
        "rateId": rateId,
    }
...
if __name__ == "__main__":
    req_dto = AccountFxRateReq(
        userNo=5181240702000026848,  # 5181240821000008798-五五 、5181240702000026848-uat桐乡
        orderNo="ORDER123456",
        business=RateBusinessTypeEnum.BUSINESS_gep,
        receiveChannel="1200923053",  # Payso-电商收款-API-1200923079   Banking Circle S.A. - E贸汇-1200923053
        # bank="24",
        ReceiveCcy="PLN",
        ReceiveAmt=Decimal("10.00"),
        accountCcy="PLN",    # 渠道入账币种
        # accountAmt=Decimal("10000"),
        accountAmt = Decimal(str(round(random.uniform(0, 599999.99), 2))),
        merchantCcy="USD"     # 商户入账币种
    )
    log_response= {"accountAmt":8900,"accountCcy":"PLN","business":"B2B_BUSINESS","merchantCcy":"USD","orderNo":"20250710154759622","receiveAmt":98900,"receiveCcy":"PLN","receiveChannel":"1200923053","userNo":5181240702000026848}
    req_dto = AccountFxRateReq(
        userNo=log_response.get("userNo"),  # 5181240821000008798-五五 、5181240628000024148-桐乡
        orderNo=log_response.get("orderNo"),
    ...
    # TODO：调用新方法一次调用2次不同的请求对象，验证缓存情况
    # execute_rate_calculations(req_dto, req_dto1)
    # 调用汇率详情查询接口-查询当时的汇率
